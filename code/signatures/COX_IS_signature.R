...
Get_COX_IS_signature = function( expr ){

	source('meta/Get_HR.R')
	source('meta/Get_DI.R')
	get_Directory( dir= "COX_IS" )
			...
			for( j in 1:length(tumor)){
				...
				if( length( geneSig[ !is.na( phenoData(expr[[i]])$os[ phenoData(expr[[i]])$primary %in% tumor[j] ] ) ] ) >= 20 ){
					...
					hr = Get_HR_dicho( surv=phenoData(expr[[i]])$os[ phenoData(expr[[i]])$primary %in% tumor[j] ] , time=phenoData(expr[[i]])$t.os[ phenoData(expr[[i]])$primary %in% tumor[j] ] ,
											 ...
											 title = paste( study[i] , tumor[j] , 36 , "OS_Overall" , sep="_" ) , ylab="Overall Survival" , dir="../results/Signature/COX_IS/KMPlot/OS" )
				...
				if( length( geneSig[ !is.na( phenoData(expr[[i]])$pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] ) ]  ) >= 20 ){
					...
					hr = Get_HR_dicho( surv=phenoData(expr[[i]])$pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] , time=phenoData(expr[[i]])$t.pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] ,
											 ...
											 title = paste( study[i] , tumor[j] , 36 , "PFS_Overall" , sep="_" ) , ylab="Progression-Free Survival" , dir="../results/Signature/COX_IS/KMPlot/PFS" )
	...
	di_os = if_NULL( x= di_os , colnames= colnames , study= costudyhstudyort )
	...
	save( log_response , log_dicho_response , file="../results/Signature/COX_IS/COX_IS_LogReg_result.RData" )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file="../results/Signature/COX_IS/COX_IS_COX_result.RData" ) 
	save(  di_os , di_pfs , file="../results/Signature/COX_IS/COX_IS_DI_result.RData" ) 
}
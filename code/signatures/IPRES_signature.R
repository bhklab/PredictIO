...
Get_IPRES_signature = function( expr ){

	source('meta/Get_HR.R')
	source('meta/Get_DI.R')
	get_Directory( dir= "IPRES" )
			...
			for( j in 1:length(tumor)){
				...
				if( length( geneSig[ !is.na( phenoData(expr[[i]])$os[ phenoData(expr[[i]])$primary %in% tumor[j] ] ) ] ) >= 20 ){
					...
					hr = Get_HR_dicho( surv=phenoData(expr[[i]])$os[ phenoData(expr[[i]])$primary %in% tumor[j] ] , time=phenoData(expr[[i]])$t.os[ phenoData(expr[[i]])$primary %in% tumor[j] ] ,
											 ...
											 title = paste( study[i] , tumor[j] , 36 , "OS_Overall" , sep="_" ) , ylab="Overall Survival" , dir="../results/Signature/IPRES/KMPlot/OS" )
				...
				if( length( geneSig[ !is.na( phenoData(expr[[i]])$pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] ) ]  ) >= 20 ){
					...
					hr = Get_HR_dicho( surv=phenoData(expr[[i]])$pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] , time=phenoData(expr[[i]])$t.pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] ,
											 ...
											 title = paste( study[i] , tumor[j] , 36 , "PFS_Overall" , sep="_" ) , ylab="Progression-Free Survival" , dir="../results/Signature/IPRES/KMPlot/PFS" )
	...
	save( log_response , log_dicho_response , file="../results/Signature/IPRES/IPRES_LogReg_result.RData" )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file="../results/Signature/IPRES/IPRES_COX_result.RData" ) 
	save(  di_os , di_pfs , file="../results/Signature/IPRES/IPRES_DI_result.RData" ) 
}
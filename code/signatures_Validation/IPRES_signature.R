...
Get_IPRES_signature = function( expr , cohort , sigType ){

	source('meta/Get_HR.R')
	source('meta/Get_DI.R')
	get_Directory( dir= "IPRES" , cohort= cohort )
	...
	if( ifelse( is.null( nrow( data[ rownames(data) %in% sig , ]) ) , 1 , nrow( data[ rownames(data) %in% sig , ] ) ) / length( sig ) > 0.8 & ncol(data) >= 10 ){
		geneSig <- apply( gsva( get_Scale( x=data ) , sig , verbose=FALSE ) , 2 , mean , na.rm=TRUE )
		names(geneSig) = colnames(data)
	}

	if( length( geneSig[ !is.na( clin$os ) ] ) >= 10 ){
		## Compute the association IPRES signature with OS (36 month cutoff) using Cox Regression Model
		...
		## Compute the association IPRES signature with OS (36 month cutoff) using Concordence Index
		...
		## Compute the association IPRES signature (HIGH vs LOW) with OS (36 month cutoff) using Cox Regression Model
		hr = Get_HR_dicho( surv=clin$os , time=clin$t.os ,
								 time_censor=36 , variable= geneSig , cutoff= median( geneSig , na.rm=TRUE ) ,
								 title = paste( cohort , primary , 36 , "OS_Overall" , sep="_" ) , ylab="Overall Survival" , dir= paste( "../results/signature_Validation/" , cohort , "/IPRES/KMPlot/OS" , sep="" ) )
	...
	if( length( geneSig[ !is.na( clin$pfs ) ]  ) >= 10 ){
		## Compute the association of IPRES signature with PFS using Cox Regression Model
		...
		## Compute the association of IPRES signature with PFS using Concordence Index
		...
		## Compute the association of IPRES signature (HIGH vs LOW) with PFS using Cox Regression Model
		hr = Get_HR_dicho( surv=clin$pfs , time=clin$t.pfs ,
								 time_censor=24 , variable= geneSig , cutoff= median( geneSig , na.rm=TRUE ) ,
								 title = paste( cohort , primary , 36 , "PFS_Overall" , sep="_" ) , ylab="Progression-Free Survival" , dir= paste( "../results/signature_Validation/" , cohort , "/IPRES/KMPlot/PFS" , sep="" ) )
	...
	if( length( geneSig[ !is.na( clin$response ) ]  ) >= 10 ){
		...
		pdf( paste( "../results/signature_Validation/" , cohort , "/IPRES/ICB_Response.pdf" , sep="" ) , height=3.5,width=3,bg="transparent")

	  		xLabels <- paste( c( "R" , "NR" ) , "\n(N=" , table(d$response) , ")" , sep = "" )
		    yLabels <- seq( round( min( d$sig , na.rm=TRUE ) , 1 ) ,  round( max( d$sig , na.rm=TRUE ) , 1 ) , by=.25 ) 
		    boxplot( sig ~ response , data=d , ylab= "IPRES" , xlab="ICB Response" , main="" , 
		...
		ggsave(filename= paste( "../results/signature_Validation/" , cohort , "/IPRES/ICB_Response_ROC.pdf" , sep="" ), plot=ROCPlot, device="pdf", height=4 , width=4 )
	...
	save( log_response , log_dicho_response , file= paste( "../results/signature_Validation/" , cohort , "/IPRES/IPRES_LogReg_result.RData" , sep="" ) )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file= paste( "../results/signature_Validation/" , cohort , "/IPRES/IPRES_COX_result.RData" , sep="" ) ) 
	save(  di_os , di_pfs , file= paste( "../results/signature_Validation/" , cohort , "/IPRES/IPRES_DI_result.RData" , sep="" ) ) 
	save(  auc , file=paste( "../results/signature_Validation/" , cohort , "/IPRES/IPRES_AUC.RData" , sep="" ) ) 

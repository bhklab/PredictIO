...
Get_IPS_signature = function( cohort , sigType ){

	source('meta/Get_HR.R')
	source('meta/Get_DI.R')
	get_Directory( dir= "IPS" , cohort= cohort )
	...
	if( ! is.null( length( IPS ) ) & length( IPS ) > 20 ){
		...
		if( length( IPS[ !is.na( clin$os ) ] ) >= 10 ){
			...
			cox_os = rbind( cox_os , 
					c( cohort , primary , "TPM" , length( IPS[ !is.na( clin$os ) ] ) ,  hr ) )
			...
			cci = Get_DI_continous( surv=clin$os , time=clin$t.os ,
									 time_censor=36 , variable= IPS )
			di_os = rbind( di_os , 
					c( cohort , primary , "TPM" , length( IPS[ !is.na( clin$os ) ] ) ,  cci ) )
			...
			hr = Get_HR_dicho( surv=clin$os , time=clin$t.os ,
									 ...
									 title = paste( cohort , primary , 36 , "OS_Overall" , sep="_" ) , ylab="Overall Survival" , dir= paste( "../results/signature_Validation/" , cohort , "/IPS/KMPlot/OS" , sep="" ) )
		...
		if( length( IPS[ !is.na( clin$pfs ) ]  ) >= 10 ){
			...
			cox_pfs = rbind( cox_pfs , 
					c( cohort , primary , "TPM" , length( IPS[ !is.na( clin$pfs ) ] ) ,  hr ) )
			...
			cci = Get_DI_continous( surv=clin$pfs , time=clin$t.pfs ,
									 time_censor=24 , variable= IPS )
			di_pfs = rbind( di_pfs , 
					c( cohort , primary , "TPM" , length( IPS[ !is.na( clin$pfs ) ] ) ,  cci ) )
			...
			hr = Get_HR_dicho( surv=clin$pfs , time=clin$t.pfs ,
									 ...
									 title = paste( cohort , primary , 36 , "PFS_Overall" , sep="_" ) , ylab="Progression-Free Survival" , dir= paste( "../results/signature_Validation/" , cohort , "/IPS/KMPlot/PFS" , sep="" ) )
		...
		if( length( IPS[ !is.na( clin$response ) ]  ) >= 10 ){
			...
			log_response = rbind( log_response , c( cohort , primary , 
						"TPM" , 
						length( IPS[ !is.na( clin$response ) ] ) , 
			...
			pdf( paste( "../results/signature_Validation/" , cohort , "/IPS/ICB_Response.pdf" , sep="" ) , height=3.5,width=3,bg="transparent")

		  		xLabels <- paste( c( "R" , "NR" ) , "\n(N=" , table(d$response) , ")" , sep = "" )
			    yLabels <- seq( round( min( d$sig , na.rm=TRUE ) , 1 ) ,  round( max( d$sig , na.rm=TRUE ) , 1 ) , by=.25 ) 
			    boxplot( sig ~ response , data=d , ylab= "IPS" , xlab="ICB Response" , main="" , 
			...
			ggsave(filename= paste( "../results/signature_Validation/" , cohort , "/IPS/ICB_Response_ROC.pdf" , sep="" ), plot=ROCPlot, device="pdf", height=4 , width=4 )
			...
			if( length(IPS[ !is.na( clin$response ) ] )>= 40 ){
	...
	save( log_response , log_dicho_response , file= paste( "../results/signature_Validation/" , cohort , "/IPS/IPS_LogReg_result.RData" , sep="" ) )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file= paste( "../results/signature_Validation/" , cohort , "/IPS/IPS_COX_result.RData" , sep="" ) ) 
	save(  di_os , di_pfs , file= paste( "../results/signature_Validation/" , cohort , "/IPS/IPS_DI_result.RData" , sep="" ) ) 
	save(  auc , file=paste( "../results/signature_Validation/" , cohort , "/IPS/IPS_AUC.RData" , sep="" ) ) 

...
Get_COX_IS_signature = function( expr , cohort , sigType ){

	source('meta/Get_HR.R')
	source('meta/Get_DI.R')
	get_Directory( dir= "COX_IS" , cohort= cohort )
	...
	if( length( geneSig[ !is.na( clin$os ) ] ) >= 10 ){
		## Compute the association COX_IS signature with OS (36 month cutoff) using Cox Regression Model
		...
		## Compute the association COX_IS signature with OS (36 month cutoff) using Concordence Index
		...
		## Compute the association COX_IS signature (HIGH vs LOW) with OS (36 month cutoff) using Cox Regression Model
		hr = Get_HR_dicho( surv=clin$os , time=clin$t.os ,
								 time_censor=36 , variable= geneSig , cutoff= median( geneSig , na.rm=TRUE ) ,
								 title = paste( cohort , primary , 36 , "OS_Overall" , sep="_" ) , ylab="Overall Survival" , dir= paste( "../results/signature_Validation/" , cohort , "/COX_IS/KMPlot/OS" , sep="" ) )
	...
	if( length( geneSig[ !is.na( clin$pfs ) ]  ) >= 10 ){
		## Compute the association of COX_IS signature with PFS using Cox Regression Model
		...
		## Compute the association of COX_IS signature with PFS using Concordence Index
		...
		## Compute the association of COX_IS signature (HIGH vs LOW) with PFS using Cox Regression Model
		hr = Get_HR_dicho( surv=clin$pfs , time=clin$t.pfs ,
								 time_censor=24 , variable= geneSig , cutoff= median( geneSig , na.rm=TRUE ) ,
								 title = paste( cohort , primary , 36 , "PFS_Overall" , sep="_" ) , ylab="Progression-Free Survival" , dir= paste( "../results/signature_Validation/" , cohort , "/COX_IS/KMPlot/PFS" , sep="" ) )
	...
	if( length( geneSig[ !is.na( clin$response ) ]  ) >= 10 ){
		...
		pdf( paste( "../results/signature_Validation/" , cohort , "/COX_IS/ICB_Response.pdf" , sep="" ) , height=3.5,width=3,bg="transparent")

	  		xLabels <- paste( c( "R" , "NR" ) , "\n(N=" , table(d$response) , ")" , sep = "" )
		    yLabels <- seq( round( min( d$sig , na.rm=TRUE ) , 1 ) ,  round( max( d$sig , na.rm=TRUE ) , 1 ) , by=.25 ) 
		    boxplot( sig ~ response , data=d , ylab= "COX_IS" , xlab="ICB Response" , main="" , 
		...
		ggsave(filename= paste( "../results/signature_Validation/" , cohort , "/COX_IS/ICB_Response_ROC.pdf" , sep="" ), plot=ROCPlot, device="pdf", height=4 , width=4 )
	...
	save( log_response , log_dicho_response , file= paste( "../results/signature_Validation/" , cohort , "/COX_IS/COX_IS_LogReg_result.RData" , sep="" ) )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file= paste( "../results/signature_Validation/" , cohort , "/COX_IS/COX_IS_COX_result.RData" , sep="" ) ) 
	save(  di_os , di_pfs , file= paste( "../results/signature_Validation/" , cohort , "/COX_IS/COX_IS_DI_result.RData" , sep="" ) ) 
	save(  auc , file=paste( "../results/signature_Validation/" , cohort , "/COX_IS/COX_IS_AUC.RData" , sep="" ) ) 

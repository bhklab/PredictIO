...
Get_IO_resistance <- function( meta_res ){
			...
			for( j in 1:length(tumor)){
				...
				if( ifelse( is.null( nrow(data[ rownames(data) %in% sig,]) ) , 1 , nrow(data[ rownames(data) %in% sig,]) ) / length( sig ) > 0.7 & ncol(data) > 20 ){
					...
					if( length( geneSig[ !is.na( phenoData(expr[[i]])$os[ phenoData(expr[[i]])$primary %in% tumor[j] ] ) ] ) >= 20 ){
						## Compute the association IO_Resistance signature with OS (36 month cutoff) using Cox Regression Model
						...
						## Compute the association IO_Resistance signature with OS (36 month cutoff) using Concordence Index
						...
						## Compute the association IO_Resistance signature (HIGH vs LOW) with OS (36 month cutoff) using Cox Regression Model
						hr = Get_HR_dicho( surv=phenoData(expr[[i]])$os[ phenoData(expr[[i]])$primary %in% tumor[j] ] , time=phenoData(expr[[i]])$t.os[ phenoData(expr[[i]])$primary %in% tumor[j] ] ,
												 ...
												 title = paste( study[i] , tumor[j] , 36 , "OS_Overall" , sep="_" ) , ylab="Overall Survival" , dir= "../results/PredictIO/IO_Resistance/KMPlot/OS" )
					...
					if( length( geneSig[ !is.na( phenoData(expr[[i]])$pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] ) ]  ) >= 20 ){
						## Compute the association of IO_Resistance signature with PFS using Cox Regression Model
						...
						## Compute the association of IO_Resistance signature with PFS using Concordence Index
						...
						## Compute the association of IO_Resistance signature (HIGH vs LOW) with PFS using Cox Regression Model
						hr = Get_HR_dicho( surv=phenoData(expr[[i]])$pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] , time=phenoData(expr[[i]])$t.pfs[ phenoData(expr[[i]])$primary %in% tumor[j] ] ,
												 ...
												 title = paste( study[i] , tumor[j] , 36 , "PFS_Overall" , sep="_" ) , ylab="Progression-Free Survival" , dir= "../results/PredictIO/IO_Resistance/KMPlot/PFS" )
					...
					if( length( geneSig[ !is.na( phenoData(expr[[i]])$response[ phenoData(expr[[i]])$primary %in% tumor[j] ] ) ]  ) >= 20 ){
						...
						## Association of IO_Resistance signature with Response (Continous)
						...
						## Association of IO_Resistance signature with Response (HIGH vs Low)
	...
	save( log_response , log_dicho_response , log_tertile_response , file= "../results/PredictIO/IO_Resistance/IO_Resistance_LogReg_result.RData" )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , cox_tertile_os , cox_tertile_pfs , file= "../results/PredictIO/IO_Resistance/IO_Resistance_COX_result.RData" ) 
	save(  di_os , di_pfs , file= "../results/PredictIO/IO_Resistance/IO_Resistance_DI_result.RData" ) 
}
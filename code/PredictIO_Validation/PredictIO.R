...
Get_PredictIO <- function( exprID , meta_res ){
	...
	IOscore = NULL
	if( ncol(expr)){ IOscore <- getIOscore( data = expr , meta_res= meta_res ) }

	primary = unique( clin$primary )
	primary = ifelse( length( primary ) > 1 , "Pan-Cancer" , primary )

	if( !is.null( IOscore ) ){
		if( length( IOscore[ !is.na( clin$os ) ] ) >= 10 ){
			...
			cox_os = rbind( cox_os , 
					c( exprID , primary , "TPM" , length( IOscore[ !is.na( clin$os ) ] ) ,  hr ) )
			...
			cci = Get_DI_continous( surv=clin$os , time=clin$t.os ,
									 time_censor=36 , variable= IOscore )
			di_os = rbind( di_os , 
					c( exprID , primary , "TPM" , length( IOscore[ !is.na( clin$os ) ] ) ,  cci ) )
			...
			hr = Get_HR_dicho( surv=clin$os , time=clin$t.os ,
									 ...
									 title = paste( exprID , 36 , "OS_Median" , sep="_" ) , ylab="Overall Survival" , dir= paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO" , sep="" ) )
			cox_dicho_os = rbind( cox_dicho_os , 
					c( exprID , primary , "TPM" , length( IOscore[ !is.na( clin$os ) ] ) ,  hr ) )

		}

		if( length( IOscore[ !is.na( clin$pfs ) ] ) >= 10 ){
			...
			cox_pfs = rbind( cox_pfs , 
					c( exprID , primary , "TPM" , length( IOscore[ !is.na( clin$pfs ) ] ) ,  hr ) )
			...
			cci = Get_DI_continous( surv=clin$pfs , time=clin$t.pfs ,
									 time_censor=24 , variable= IOscore )
			di_pfs = rbind( di_pfs , 
					c( exprID , primary , "TPM" , length( IOscore[ !is.na( clin$pfs ) ] ) ,  cci ) )
			...
			hr = Get_HR_dicho( surv=clin$pfs , time=clin$t.pfs ,
									 ...
									 title = paste( exprID , 36 , "PFS_Median" , sep="_" ) , ylab="Progression-Free Survival" , dir= paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO" , sep="" ) )
		...
		if( length( IOscore[ !is.na( clin$response ) ]  ) >= 10 ){
			...
			log_response = rbind( log_response , c( exprID , primary , 
						"TPM" , 
						length( IOscore[ !is.na( clin$response ) ]  ) , 
			...
			pdf( paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO/ICB_Response.pdf" , sep="" ) , height=3.5,width=3,bg="transparent")
			...
			################################################################
			################################################################
			x = ifelse( clin$response %in% "R" , 1 , ifelse( clin$response %in% "NR" , 0 , NA ) )
			...
			ggsave(filename= paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO/ICB_Response_ROC.pdf" , sep="" ), plot=ROCPlot, device="pdf", height=4 , width=4 )
	...
	save( log_response , log_dicho_response , file=paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_LogReg_result.RData" , sep="" ) )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file=paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_COX_result.RData" , sep="" ) ) 
	save(  di_os , di_pfs , file=paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_DI_result.RData" , sep="" ) ) 
	save(  auc , file=paste( "../results/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_AUC.RData" , sep="" ) ) 
}
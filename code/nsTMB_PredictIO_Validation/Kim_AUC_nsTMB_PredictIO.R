...
getPredictIO <- function( data , meta_res ){
...
snv = as.data.frame( fread( "../data/validation_cohort/snv/Kim.csv.gz" , sep=";" , stringsAsFactors=FALSE ))
tmb = table( snv$Sample ) / 50 

load("../data/validation_cohort/Kim.RData")

file = "../results/nsTMB_PredictIO_Validation/"
dir.create( file )
dir.create( paste( file , "/Kim" , sep="" ) )

response = clin$response
names(response) = rownames(clin)

PredictIO = NULL
if( ncol(expr)){ PredictIO <- getPredictIO( data = as.matrix( expr ) , meta_res ) }
names(PredictIO) = colnames( expr )

missing_patient = names(PredictIO)[ !names(PredictIO) %in% names(tmb) ]
...
patient = intersect( intersect( names( response ) , names( PredictIO ) ) , names( tmb ) )
response = response[ patient ]
PredictIO = PredictIO[ patient ]
tmb = tmb[ patient ]

x = ifelse( response %in% "R" , 1 , ifelse( response %in% "NR" , 0 , NA ) )

fit = glm( x ~ PredictIO , family = binomial( link="logit" ) )

d = as.data.frame( cbind( x , PredictIO ) )
...
pdf( "../results/nsTMB_PredictIO_Validation/Kim/PredictIO_Response.pdf" , height=3.5,width=3,bg="transparent")
...
ggsave(filename= "../results/nsTMB_PredictIO_Validation/Kim/PredictIO_ROC.pdf" , plot=ROCPlot, device="pdf", height=4 , width=4 )

n_patient = length( PredictIO )
save( auc , n_patient , file = "../results/nsTMB_PredictIO_Validation/Kim/PredictIO_AUC.RData" )
...
pdf( "../results/nsTMB_PredictIO_Validation/Kim/tmb_Response.pdf" , height=3.5,width=3,bg="transparent")
...
ggsave(filename= "../results/nsTMB_PredictIO_Validation/Kim/tmb_ROC.pdf" , plot=ROCPlot, device="pdf", height=4 , width=4 )

n_patient = length( tmb )
save( auc , n_patient , file = "../results/nsTMB_PredictIO_Validation/Kim/tmb_AUC.RData" )


################################################################
################################################################

dat = as.data.frame( cbind( response , PredictIO , tmb ) )
dat$response = as.numeric( as.character( ifelse( dat$response %in% "R" , 1 , ifelse( dat$response %in% "NR" , 0 , NA ) ) ) )
dat$PredictIO = as.numeric( as.character( dat$PredictIO ) )
...
pdf( "../results/nsTMB_PredictIO_Validation/Kim/ROCcurve_PredictIO_TMB.pdf" , height=4,width=4,bg="transparent")

	roc( logit$y[ logit$sig %in% "PredictIO" ] , logit$fitted.values[ logit$sig %in% "PredictIO" ] , plot=TRUE, legacy.axes=FALSE, percent=TRUE, col="#E53935", lwd=2, print.auc=TRUE)
...
table(ifelse( PredictIO < median( PredictIO , na.rm=TRUE ) , 0 , 1 ) , response )

table(ifelse( tmb < 10 , 0 , 1 ) , response )
table(ifelse( PredictIO < quantile( PredictIO , na.rm=TRUE , probs=.66 ) , 0 , 1 ) , response )
